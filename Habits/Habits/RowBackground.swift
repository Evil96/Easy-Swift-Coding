...
struct RowBackground: View {
    ...
    init(width: CGFloat, height: CGFloat, radius: CGFloat = 20, bgColor: Color = .red, curveColor: Color = .green) {
        self.width = width
        self.height = height
        self.radius = radius
    ...
    var body: some View {
        ZStack {
            ...
            ForEach(0 ..< 3) { _ in
                figS(firstX: self.x_1, secondX: self.x_2, starting: self.generateStarting(), ending: self.generateEnding())
    ...
    func generateStarting() -> CGFloat {
        return 0.5 + CGFloat.random(in: 0...1)
    }
    
    func generateEnding() -> CGFloat {
        return 1.0 + CGFloat.random(in: 0...1.5)
    }
}

struct CardBackgroundView_Previews: PreviewProvider {
    static var previews: some View {
        RowBackground(width: 150, height: 150)
    }
}
